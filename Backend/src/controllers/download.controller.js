...
function codeBlock(doc, text, language = "") {
  ...
  // Draw background
  doc.rect(x - 5, y - 4, width + 10, height).fill(COLORS.codebg);
...
function conceptCard(doc, title, text, type = "info") {
  ...
  doc.roundedRect(x, y, width, totalHeight, LAYOUT.cardRadius).fill(scheme.bg);

  // Left border accent
  doc.rect(x, y, 4, totalHeight).fill(scheme.border);
...
function tableOfContents(doc, content) {
  ...
  lines.forEach((line) => {
    ...
    if (match) {
      ...
      if (level <= 2) {
        // Only include h1 and h2 for cleaner TOC
        toc.push({ level, text });
      }
    }
  });
...
function revisionSection(doc, points) {
  ...
  points.forEach((point, i) => {
    ...
    doc.font("Regular").fontSize(11.5).fillColor(COLORS.text);
...
function renderShortQuestions(doc, questions) {
  ...
  questions.forEach((q, i) => {
    ...
    doc.rect(x, y, width, answerHeight).stroke(COLORS.divider);
...
function addFooters(doc, title) {
  ...
  for (let i = 0; i < pages.count; i++) {
    ...
    if (i > 0) {
      ...
      doc.text(shortTitle, doc.page.margins.left, footerY, {
        lineBreak: false,
      });
    }
    ...
    doc.text(pageNum, (doc.page.width - pageNumWidth) / 2, footerY, {
      lineBreak: false,
    });
    ...
    doc.text(
      watermark,
      doc.page.width - doc.page.margins.right - watermarkWidth,
      footerY,
      { lineBreak: false }
    );
  }
...
/**
 * Generate and download PDF for a saved note by its ID
 * @param {Request} req - Express request object (expects req.params.noteId)
 * @param {Response} res - Express response object
 */
const downloadByNoteId = async (req, res) => {
  ...
  try {
    ...
    // Check for POST request with result object (immediate download after generation)
    if (req.body && req.body.result) {
      result = req.body.result;
    }
    // Check for GET request with noteId (profile page download)
    else if (req.params && req.params.noteId) {
      ...
      if (!noteId) {
        return res.status(400).json({
          success: false,
          error: "Invalid request: 'noteId' param is required",
        });
      }
      ...
      logPerformance("Note fetch from database", startTime);

      // Saved notes are stored as JSON string in note.content
      let parsedSavedResult = null;
      if (typeof note.content === "string") {
        try {
          parsedSavedResult = JSON.parse(note.content);
        } catch (e) {
          parsedSavedResult = null;
        }
      } else if (note.content && typeof note.content === "object") {
        parsedSavedResult = note.content;
      }

      // Build result object so extraction flow matches pdf.controller.js
      result = {
        ...(parsedSavedResult && typeof parsedSavedResult === "object" ? parsedSavedResult : {}),
        topic: note.topic || (parsedSavedResult && parsedSavedResult.topic) || "AI Generated Notes",
        title: note.title || (parsedSavedResult && parsedSavedResult.title),
        content:
          (parsedSavedResult &&
            (parsedSavedResult.content ||
              parsedSavedResult.notes ||
              parsedSavedResult.markdown ||
              parsedSavedResult.text)) ||
          note.summary ||
          "",
        summary: (parsedSavedResult && parsedSavedResult.summary) || note.summary || "",
      };
    }
    // Neither POST result nor GET noteId found
    else {
      return res.status(400).json({
        success: false,
        error: "Invalid request: 'result' object or 'noteId' param is required",
      });
    }

    // Extract and sanitize data - exact structure used in pdf.controller.js
    ...
    // Use topic/title as filename (preserve readable topic text)
    const safeTitle = cleanText(title || "AI Generated Notes")
      .replace(/[<>:"/\\|?*\u0000-\u001F]/g, "")
      .replace(/\s+/g, " ")
      .trim()
      .substring(0, 100) || "AI Generated Notes";
