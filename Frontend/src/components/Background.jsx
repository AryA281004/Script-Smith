...
uniform bool enableExtra;

uniform int topLineCount;
uniform int middleLineCount;
uniform int bottomLineCount;
uniform int extraLineCount;

uniform float topLineDistance;
uniform float middleLineDistance;
uniform float bottomLineDistance;
uniform float extraLineDistance;

uniform vec3 topWavePosition;
uniform vec3 middleWavePosition;
uniform vec3 bottomWavePosition;
uniform vec3 extraWavePosition;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (enableExtra) {
    for (int i = 0; i < extraLineCount; ++i) {
      ...
      float angle = extraWavePosition.z * log(length(baseUv) + 1.0);
      vec2 ruv = baseUv * rotate(angle);
      col += lineCol * wave(
        ruv + vec2(extraLineDistance * fi + extraWavePosition.x, extraWavePosition.y),
        0.8 + 0.25 * fi,
        baseUv,
        mouseUv,
        interactive
...
const Background = ({
  ...
  extraWavePosition = { x: 0.01, y: -0.35, rotate: 6.45 },
  animationSpeed = 1,
  interactive = true,
  bendRadius = 5.0,
  bendStrength = -0.5,
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full h-full overflow-hidden floating-lines-container ${className}`}
      style={{
